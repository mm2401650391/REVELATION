...
// ═══════════════════════════════════════════════════════════════
//  12. Corpus 卡片掉落测试（集气城市）
// ═══════════════════════════════════════════════════════════════
// 用法：testDropCard('c_zone1_gas')  — 从集气城市卡组随机掉一张
// 用法：testDropCard('c_corpus_01')  — 指定 codexId 掉落（测试碎片）
function testDropCard(deckIdOrCodexId, forceChance) {
    var card = null;
    if (typeof DECK_CARDS !== 'undefined' && DECK_CARDS[deckIdOrCodexId]) {
        var deck = DECK_CARDS[deckIdOrCodexId];
        card = deck[Math.floor(Math.random() * deck.length)];
    } else if (typeof findCardById === 'function') {
        card = findCardById(deckIdOrCodexId);
    }
    if (!card) { console.error('未找到卡片:', deckIdOrCodexId); return; }

    if (typeof addPlayerCard === 'function') {
        var result = addPlayerCard(card);
        console.log('🎴 测试掉落:', card.name, '(id:' + card.id + ')', 'rarity:', card.rarity, '→ isNew:', result.isNew, 'count:', result.count);
        if (typeof showToast === 'function') {
            showToast('测试掉落: ' + card.name + (result.isNew ? ' ✨新卡!' : ' → 碎片'), result.isNew ? 'success' : 'info');
        }
    } else {
        console.log('🎴 测试掉落(无addPlayerCard):', card);
    }
    return card;
}
window.testDropCard = testDropCard;

// 批量测试掉落：testDropBatch('c_zone1_gas', 20)
function testDropBatch(deckId, count) {
    count = count || 10;
    for (var i = 0; i < count; i++) {
        (function(idx) {
            setTimeout(function() { testDropCard(deckId); }, idx * 200);
        })(i);
    }
    console.log('🎴 批量测试: ' + deckId + ' x' + count + '，间隔200ms');
}
window.testDropBatch = testDropBatch;

// ═══ 蓝图掉落测试 ═══
// testBlueprintDrop() — 模拟1次Boss战蓝图掉落（10%概率）
function testBlueprintDrop(count) {
    count = count || 1;
    var results = { total: count, dropped: 0, details: [] };

    if (typeof BATTLE_DROPS === 'undefined') {
        console.error('BATTLE_DROPS 未定义'); return;
    }

    var pool = [];
    for (var i = 0; i < BATTLE_DROPS.length; i++) {
        var d = BATTLE_DROPS[i];
        if (d.type === 'blueprint' && d.bossOnly && d.zoneFilter && d.zoneFilter.indexOf('v_zone1') !== -1) {
            pool.push(d);
        }
    }

    for (var j = 0; j < count; j++) {
        if (Math.random() < 0.10 && pool.length > 0) {
            var sel = pool[Math.floor(Math.random() * pool.length)];
            results.dropped++;
            results.details.push(sel.name);
            if (typeof addToWarehouse === 'function') {
                addToWarehouse(sel.name, sel.icon, 1, 'blueprint');
            }
        }
    }

    var msg = '📜 蓝图掉落测试: ' + count + '次Boss战 → 掉落' + results.dropped + '张';
    if (results.details.length > 0) {
        msg += ' [' + results.details.join(', ') + ']';
    }
    console.log(msg);
    if (typeof showToast === 'function') {
        showToast(msg, results.dropped > 0 ? 'success' : 'info');
    }

    var el = document.getElementById('blueprintTestResult');
    if (el) {
        var rate = count > 0 ? (results.dropped / count * 100).toFixed(1) : 0;
        el.style.display = 'block';
        el.innerHTML = '<div style="color:#aaa;">模拟 ' + count + ' 次Boss战 (10%掉率)</div>' +
            '<div style="color:' + (results.dropped > 0 ? '#ffcc66' : '#888') + ';">实际掉落: ' + results.dropped + '/' + count + ' (' + rate + '%)</div>' +
            (results.details.length > 0 ? '<div style="color:#c8a84b;margin-top:4px;">' + results.details.join(', ') + '</div>' : '');
    }
}
window.testBlueprintDrop = testBlueprintDrop;
