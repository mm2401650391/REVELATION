...
window.enterMarooMarket = function() {
	...
	var echoEntrance = document.getElementById('echoShopEntrance');
	if (echoEntrance) echoEntrance.style.display = 'none';
	var echoShopPage = document.getElementById('page-echoshop');
	if (echoShopPage) echoShopPage.style.display = 'none';
...
window.backToPrimeShop = function() {
	...
	if (echoShopPage) echoShopPage.style.display = 'none';
	var echoEntrance = document.getElementById('echoShopEntrance');
	if (echoEntrance) echoEntrance.style.display = 'block';
	updateMarooPoints();
};

// 全局暴露 enterEchoShop
window.enterEchoShop = function() {
	document.getElementById('primeShopArea').style.display = 'none';
	document.getElementById('marooMarketEntrance').style.display = 'none';
	var echoEntrance = document.getElementById('echoShopEntrance');
	if (echoEntrance) echoEntrance.style.display = 'none';
	var echoShopPage = document.getElementById('page-echoshop');
	if (echoShopPage) echoShopPage.style.display = 'block';
...
function isAdmin() {
    ...
    if (!currentUser.clan_rank) return false;
    return currentUser.clan_rank.toLowerCase() === 'admin';
}
...
async function redeemCodeServer(code) {
    ...
    try {
        ...
        // redeem_code RPC 只返回 {success:true}，没有奖励信息
        // 需要前端自己查询 redeem_codes 表获取奖励信息
        var codeRecord = null;
        try {
            var codeUpper = code.toUpperCase().trim();
            var { data: crData, error: crErr } = await sb.from('redeem_codes')
                .select('reward_type, reward_value, echo_pack_deck_id')
                .eq('code', codeUpper)
                .maybeSingle();
            if (!crErr && crData) {
                codeRecord = crData;
                console.log('[redeem] 查询兑换码记录:', JSON.stringify(codeRecord));
            } else if (crErr) {
                console.warn('[redeem] 查询兑换码记录失败:', crErr);
            }
        } catch (qErr) {
            console.warn('[redeem] 查询兑换码异常:', qErr);
        }

        // 合并 RPC 结果和查询结果（优先查询结果）
        if (codeRecord) {
            result.reward_type = codeRecord.reward_type;
            result.reward_value = codeRecord.reward_value;
            result.echo_pack_deck_id = codeRecord.echo_pack_deck_id;
        }
        ...
        // 回响盲盒兑换
        if (typeof handleRedeemEchoPack === 'function') {
            const echoPackResult = handleRedeemEchoPack(result);
            if (echoPackResult) {
                rewards.push('🎁 回响盲盒');
            }
        }

        // 回响碎片兑换 (reward_type 格式: echo_fragment_1 ~ echo_fragment_5)
        var isEchoFrag = false;
        if (result.reward_type) {
            var rt = String(result.reward_type);
            console.log('[redeem] reward_type 原始值:', rt, '| reward_value:', result.reward_value);
            if (rt.indexOf('echo_fragment_') === 0 || rt === 'echo_fragment') {
                isEchoFrag = true;
            }
        }
        // 兜底：检查 result 是否有碎片相关字段的其他写法
        if (!isEchoFrag && result.echo_fragment_type) {
            console.log('[redeem] 检测到 echo_fragment_type 字段，尝试处理');
            var fragRarity = parseInt(result.echo_fragment_type) || 1;
            var fragAmount = parseInt(result.reward_value) || parseInt(result.echo_fragment_amount) || 1;
            var fragments = (gameData && gameData.echo_fragments) || {};
            var fKey = String(fragRarity);
            fragments[fKey] = (fragments[fKey] || 0) + fragAmount;
            if (gameData) gameData.echo_fragments = fragments;
            try { localStorage.setItem('echo_fragments_' + currentUser.id, JSON.stringify(fragments)); } catch(e) {}
            // 统一走 saveGameData 写入数据库，避免与 merge_game_data RPC 竞争
            try { await saveGameData(); } catch(e) { console.error('[redeem] saveGameData 失败:', e); }
            if (typeof renderEchoShop === 'function') renderEchoShop();
            if (typeof updateEchoFragmentBar === 'function') updateEchoFragmentBar();
            var fragInfo = (typeof ECHO_FRAGMENTS !== 'undefined') ? ECHO_FRAGMENTS[fragRarity] : null;
            rewards.push('◆+' + fragAmount + ' ' + (fragInfo ? fragInfo.name : '碎片'));
        }
        if (isEchoFrag) {
            var fragRarity = 1;
            if (String(result.reward_type).indexOf('echo_fragment_') === 0) {
                fragRarity = parseInt(String(result.reward_type).split('_')[2]) || 1;
            }
            var fragAmount = parseInt(result.reward_value) || 1;
            // 直接操作 gameData
            var fragments = (gameData && gameData.echo_fragments) || {};
            var fKey = String(fragRarity);
            fragments[fKey] = (fragments[fKey] || 0) + fragAmount;
            ...
            // localStorage 备份
            try {
                localStorage.setItem('echo_fragments_' + currentUser.id, JSON.stringify(fragments));
            } catch(e) {}
            // 统一走 saveGameData 写入数据库，避免与 merge_game_data RPC 竞争
            try { await saveGameData(); } catch(e) { console.error('[redeem] saveGameData 失败:', e); }
            if (typeof renderEchoShop === 'function') renderEchoShop();
...
var _SETTINGS_LS_KEY = 'wf_settings_v1';

function initSettings() {
    if (!gameData) gameData = {};
    var saved = null;
    try { saved = JSON.parse(localStorage.getItem(_SETTINGS_LS_KEY)); } catch(e) {}
    if (saved && typeof saved === 'object') {
        gameData.settings = saved;
    } else if (!gameData.settings) {
        gameData.settings = {
                vfxIntensity: 'high',
                uiTheme: 'default',
    ...
    applyUITheme(gameData.settings.uiTheme || 'default');
}

function getSettings() {
    if (!gameData || !gameData.settings) initSettings();
    return gameData.settings;
}

function persistSettings() {
    var s = getSettings();
    try { localStorage.setItem(_SETTINGS_LS_KEY, JSON.stringify(s)); } catch(e) {}
    if (typeof saveGameData === 'function') { try { saveGameData(); } catch(e) {} }
}
...
function showSettingsModal() {
    ...
    var vfxOptions = [
        { key: 'low', label: '低', title: '精简特效', color: '#4eff4e' },
        { key: 'medium', label: '中', title: '平衡', color: '#ffaa00' },
        { key: 'high', label: '高', title: '全特效', color: '#ff4444' }
    ];
    var vfxHtml = vfxOptions.map(function(o) {
        var currentVfx = s.vfxIntensity || 'high';
        var active = currentVfx === o.key ? 'background: ' + o.color + '; color: #000; border-color: ' + o.color + '; box-shadow: 0 0 8px ' + o.color + ';' : 'border-color: #444; color: #888;';
        return '<button class="btn" onclick="setVfxIntensity(\'' + o.key + '\')" title="' + o.title + '" style="flex:1; font-size:0.8rem; padding:8px 4px; ' + active + '">' + o.label + '</button>';
    }).join('');

    var dmgCheck = s.showDamageNumbers ? '✅' : '⬜';
    var soundCheck = s.soundEnabled ? '✅' : '⬜';

    modalTitle.textContent = '⚙️ 设置';
    modalContent.innerHTML =
        '<div style="width: 100%; max-width: 420px;">' +
            '<div style="margin-bottom: 20px;">' +
                '<div style="color: var(--tenno-gold); font-family: \'Orbitron\'; font-size: 0.95rem; margin-bottom: 10px;">🎆 特效强度</div>' +
                '<div style="display: flex; gap: 8px; flex-wrap: wrap;">' + vfxHtml + '</div>' +
            '</div>' +
            '<div style="margin-bottom: 15px;">' +
                '<div style="color: var(--tenno-gold); font-family: \'Orbitron\'; font-size: 0.95rem; margin-bottom: 10px;">🔧 其他选项</div>' +
                '<div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px;">' +
                    '<button class="btn" onclick="toggleDamageNumbers()" style="border-color: #444; color: #ccc; font-size: 0.85rem; padding: 10px;">' + dmgCheck + ' 显示伤害数字</button>' +
                    '<button class="btn" onclick="toggleSound()" style="border-color: #444; color: #ccc; font-size: 0.85rem; padding: 10px;">' + soundCheck + ' 游戏音效</button>' +
                    '<button class="btn" onclick="showRedeemCodeModal()" style="background: linear-gradient(135deg, var(--tenno-gold-dim), var(--tenno-gold)); color: var(--void-black); font-size: 0.85rem; padding: 10px;">🎁 兑换码</button>' +
                    '<button class="btn" onclick="logout()" style="background: rgba(255,68,68,0.2); border-color: var(--grineer-red); color: var(--grineer-red); font-size: 0.85rem; padding: 10px;">🚪 退出登录</button>' +
                '</div>' +
            '</div>' +
            '<div style="text-align: center; margin-top: 15px; color: #666; font-size: 0.75rem;">设置已保存到本地设备</div>' +
        '</div>';
...
function setVfxIntensity(level) {
    setSetting('vfxIntensity', level);
    showSettingsModal();
    showToast('特效强度已设置为：' + (level === 'low' ? '低' : level === 'medium' ? '中' : '高'), 'success');
}

function toggleDamageNumbers() {
    var s = getSettings();
    s.showDamageNumbers = !s.showDamageNumbers;
    persistSettings();
    showSettingsModal();
    showToast(s.showDamageNumbers ? '已开启伤害数字显示' : '已关闭伤害数字显示', 'success');
}
...
function toggleSound() {
    ...
    persistSettings();
    showSettingsModal();
    showToast(s.soundEnabled ? '已开启游戏音效' : '已关闭游戏音效', 'success');
}
        ...
        '<div style="min-width: 400px;">' +
            '<div style="display: flex; gap: 10px; margin-bottom: 20px; flex-wrap:wrap;">' +
                ...
                '<button class="btn" onclick="loadRedeemStats()" style="background: linear-gradient(135deg, var(--orokin-cyan-dim), var(--orokin-cyan));">' +
                    '📊 查看统计' +
                '</button>' +
                '<button class="btn" onclick="showInactiveCleanup()" style="background: linear-gradient(135deg, var(--grineer-red), #ff6666); color: #fff;">' +
                    '🧹 清理10天未登录' +
                '</button>' +
            '</div>' +
...
function showAddCodeForm() {
    ...
    adminContent.innerHTML = 
        '<div style="background: rgba(0,0,0,0.3); padding: 20px; border-radius: 10px; border: 1px solid #333;">' +
            ...
            '</div>' +
            '<div class="input-group" style="margin-bottom: 12px;">' +
                '<label style="color: #888; font-size: 0.8rem;">奖励类型 *</label>' +
                '<select id="adminRewardType" onchange="onRewardTypeChange()" style="background:#1a1a2e;color:#fff;border:1px solid #333;padding:8px;border-radius:6px;width:100%;">' +
                    '<option value="credits">💰 Rout（虚空币）</option>' +
                    '<option value="points">💎 Prime（积分）</option>' +
                    '<option value="stamina">⚡ 负荷恢复</option>' +
                    '<option value="vip">👑 VIP天数</option>' +
                    '<option value="echo_pack">🎁 回响盲盒</option>' +
                    '<option value="echo_fragment">◆ 回响碎片</option>' +
                    '<option value="multi">📦 组合奖励</option>' +
                '</select>' +
            '</div>' +
            '<div id="rewardAmountField" style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin-bottom: 12px;">' +
                '<div class="input-group">' +
                    '<label style="color: #888; font-size: 0.8rem;">奖励数量</label>' +
                    '<input type="number" id="adminRewardAmount" value="100" min="1">' +
                '</div>' +
                '<div class="input-group">' +
                ...
                '</div>' +
            '</div>' +
            '<div id="echoPackField" style="display:none; margin-bottom:12px;" class="input-group">' +
                '<label style="color: #888; font-size: 0.8rem;">盲盒卡组</label>' +
                '<select id="adminEchoPackDeck" style="background:#1a1a2e;color:#fff;border:1px solid #333;padding:8px;border-radius:6px;width:100%;margin-bottom:8px;">' +
                    '<option value="e_zone1">☿️游掠凶形</option>' +
                    '<option value="e_zone2">☿交锋异士</option>' +
                    '<option value="e_zone3">☿暴戾战将</option>' +
                    '<option value="e_zone4">☸畸变造物</option>' +
                    '<option value="e_zone5">☿长空掠影</option>' +
                    '<option value="e_zone6">☿澜下械躯</option>' +
                    '<option value="e_zone7">☿工坊役众</option>' +
                    '<option value="e_zone8">☉统御凶僚</option>' +
                    '<option value="m_zone1">☿️晨潮矿坑</option>' +
                    '<option value="m_zone2">☿️冷却液与矿尘</option>' +
                    '<option value="m_zone3">☿️Infested摇篮</option>' +
                    '<option value="g_zone1">☿地核裂谷</option>' +
                    '<option value="g_zone2">☌异星生态</option>' +
                '</select>' +
                '<label style="color: #888; font-size: 0.8rem;">盒子类型</label>' +
                '<select id="adminEchoPackType" style="background:#1a1a2e;color:#fff;border:1px solid #333;padding:8px;border-radius:6px;width:100%;">' +
                    '<option value="0">◆遗落之盒</option>' +
                    '<option value="1">◆异变之盒</option>' +
                    '<option value="2">◆古神之盒</option>' +
                    '<option value="3">◆混沌之盒</option>' +
                    '<option value="4">◆终焉之盒</option>' +
                '</select>' +
            '</div>' +
            ...
            '<div id="expiryField" class="input-group" style="margin-bottom: 15px;">' +
                ...
                '<input type="number" id="adminExpiryDays" value="0" min="0">' +
            '</div>' +
...
function onRewardTypeChange() {
    var type = document.getElementById('adminRewardType').value;
...
async function submitNewCode() {
    ...
    var rewardAmount = parseInt(document.getElementById('adminRewardAmount').value) || 0;
    var maxUses = parseInt(document.getElementById('adminMaxUses').value) || 1;
    var rewardType = document.getElementById('adminRewardType').value;
    var echoPackDeck = document.getElementById('adminEchoPackDeck') ? document.getElementById('adminEchoPackDeck').value : '';
    var expiryDays = parseInt(document.getElementById('adminExpiryDays').value) || 0;
    ...
    try {
        ...
        // 根据 rewardType 设置对应字段
        if (rewardType === 'credits') {
            insertData.credits = rewardAmount;
        } else if (rewardType === 'points') {
            insertData.points = rewardAmount;
        } else if (rewardType === 'stamina') {
            insertData.stamina = rewardAmount;
        } else if (rewardType === 'echo_pack') {
    ...
    today_points: 0
            ...
            showLoading(false);
				...
				function switchToWarframe(warframeKey) {
					...
					var STAMINA_BATTLE_COST = 5;
					// 获取当前战斗的 stamina 消耗（星域凶顽 kuva 消耗20，其他消耗5）
					function getCurrentBattleStaminaCost() {
					    if (selectedPlanet && selectedPlanet.id === 'kuva') return 20;
...
window.recordCashEarned = function(amount) {
    ...
    gameData.today_cash_earned += amount;
					...
					function syncAllStaminaDisplays() {
					    ...
					    // 蔚蓝商栈负荷
					    var corpusStamina = document.getElementById('corpusStaminaValue');
					    if (corpusStamina) {
					        corpusStamina.textContent = stamina + '/100';
					    }
					    // 寰宇-Corpus区域负荷
					    var corpusZoneStamina = document.getElementById('corpusZoneStaminaValue');
					    if (corpusZoneStamina) {
					        corpusZoneStamina.textContent = stamina + '/100';
					    }
					}
...
setTimeout(createStars, 1000);
					...
					async function loadGameData(user) {
                        ...
                        // 确保 echo_fragments 字段存在
                        gameData.echo_fragments = gameData.echo_fragments || {};
                        // 确保 warframe_card_claims 字段存在
                        gameData.warframe_card_claims = gameData.warframe_card_claims || {};

                        // 调试日志：帮助确认数据库读取的字段
                        console.log('[loadGameData] 数据库 echo_fragments:', JSON.stringify(gameData.echo_fragments));
                        console.log('[loadGameData] 数据库 warframe_card_claims:', JSON.stringify(gameData.warframe_card_claims));
                        ...
                        if (typeof record?.today_cash_earned !== 'undefined') {
					        ...
					        // 战甲等级奖励检查（确保在数据加载完成后执行）
					        if (typeof checkWarframeLevelRewards === 'function') {
					            setTimeout(function() {
					                checkWarframeLevelRewards();
					            }, 500);
					        }
...
today_stats: todayStats,
            ...
            echo_fragments: gameData.echo_fragments || {},
            warframe_card_claims: gameData.warframe_card_claims || {},
			codex_deck_rewards: gameData.codex_deck_rewards || {},  
        };

        // 调试日志：帮助确认关键字段是否正确传递
        console.log('[saveGameData] echo_fragments:', JSON.stringify(safeGameData.echo_fragments));
        console.log('[saveGameData] warframe_card_claims:', JSON.stringify(safeGameData.warframe_card_claims));
        console.log('[saveGameData] warehouse items:', (safeGameData.warehouse || []).length);
        ...
        try {
            ...
            if (!rpcError) {
                saveSuccess = true;
                console.log('[saveGameData] merge_game_data RPC 成功');
            } else {
			    ...
			    setTimeout(function() {
					...
					async function handleLogin() {
						...
						try {
							...
							// 清除残留的碎片缓存（防止读到上一个用户的数据）
							try { localStorage.removeItem('echo_fragments_guest'); } catch(e) {}
					...
					async function logout() {
						await saveGameData();
						localStorage.removeItem('clan_user_id');
						// 清除当前用户的碎片缓存，防止切换账号时泄漏
						if (currentUser && currentUser.id) {
							localStorage.removeItem('echo_fragments_' + currentUser.id);
							localStorage.removeItem('stamina_' + currentUser.id);
...
async function claimDailyLoginReward() {
    ...
    // 更新连续签到天数（手动签到时计算）
    var lastDate = gameData.last_login_reward_date || '';
    if (lastDate && lastDate !== '1970-01-01') {
        ...
        // 首次签到或从测试数据恢复
        if (!gameData.streak || gameData.streak < 1) gameData.streak = 1;
    }
...
async function checkDailyReset() {
    ...
    if (!lastDate) {
        // 首次登录：不自动签到，设为过去日期让用户手动点击"立即签到"
        gameData.last_login_reward_date = '1970-01-01';
        await saveGameData();
        return;
    }

    // 服务器日期 > 上次领取日期 = 跨天，重置每日数据
    if (serverDate > lastDate) {
        ...
        await saveGameData();
        // 不再自动签到，用户需手动点击"立即签到"按钮领取奖励
        // streak 在 claimDailyLoginReward 中根据签到记录计算
    }
}
...
function showWeeklyLoginRewardModal() {
    ...
    overlay.addEventListener('click', function(e) {
        if (e.target === overlay) overlay.remove();
					...
					function switchPage(page) {
						...
						// 同步 window.battlePageEntered ↔ 局部变量
						if (window.battlePageEntered && !battlePageEntered) battlePageEntered = true;
						if (!window.battlePageEntered && battlePageEntered) window.battlePageEntered = true;

						// ═══ 战斗防中断：战斗进行中时保持肃清页面 ═══
						var isInCombat = (typeof autoBattleState !== 'undefined' && autoBattleState && autoBattleState.active)
						    || (typeof SkillCombat !== 'undefined' && SkillCombat && SkillCombat.isActive);
						if (isInCombat) {
						    battlePageEntered = true;
						    window.battlePageEntered = true;
						}
						...
						if (page === 'planetselect') {
							...
							if (battlePageEntered) {
								...
								if (autoBattleState.active) {
									...
									// 普通状态，未在自动肃清
									var isCorpus = window._corpusMissionActive === true;
									if (startBtn) {
										startBtn.style.display = isCorpus ? 'none' : 'block';
										startBtn.textContent = '🌍 肃清导航';
									}
									if (continueBtn) {
										continueBtn.style.display = (isCorpus || !selectedPlanet) ? 'none' : 'block';
										if (!isCorpus) {
											continueBtn.classList.remove('btn-auto-mode');
											continueBtn.disabled = false;
											var cost = getCurrentBattleStaminaCost();
											document.getElementById('continueBattleBtnText').textContent = '⚔️ 手动(-' + cost + '⚡)';
										}
									}
									if (stopBtn) {
						...
						// 对于已合并到探索的子页面，保持探索高亮
						if (!matched && (page === 'mining' || page === 'gathering')) {
							for (let i = 0; i < allNavItems.length; i++) {
								const onclick = allNavItems[i].getAttribute('onclick');
								if (onclick && onclick.includes("'explore'")) {
									allNavItems[i].classList.add('active');
									break;
								}
							}
						...
						if (page === 'customization' && typeof renderThemeSelector === 'function') renderThemeSelector();
						if (page === 'shop') {
						    renderShop();
						    renderStaminaShop();
    ...
    renderMusicConsole();
						...
						if (page === 'admintest') {
							// admin 测试页面：显示/隐藏取消无限负荷按钮
							var cancelWrap = document.getElementById('adminCancelUnlimitedWrap');
							if (cancelWrap) {
								cancelWrap.style.display = (typeof unlimitedStaminaBuff !== 'undefined' && unlimitedStaminaBuff.active) ? 'block' : 'none';
							}
						}
						if (page === 'codex') {
						    initPlayerCards();
    ...
    gameData.today_cash_earned = gameData.today_cash_earned || 0;
					...
					function adminRestoreStamina() {
					    modifyStamina(STAMINA_MAX - stamina, true);
					    showToast('负荷已恢复！当前: ' + stamina + '/' + STAMINA_MAX, 'success');
					}

					function adminCancelUnlimitedStamina() {
					    deactivateUnlimitedStamina();
					    clearUnlimitedStaminaState();
					    localStorage.removeItem('unlimited_stamina_end_' + (currentUser ? currentUser.id : ''));
					    localStorage.removeItem('unlimited_stamina_local_' + (currentUser ? currentUser.id : ''));
					    showToast('⏹ 无限负荷效果已取消', 'success');
					    // 隐藏取消按钮
					    var cancelWrap = document.getElementById('adminCancelUnlimitedWrap');
...
function startAutoBattleWithPlanet(planet, skipStaminaDeduction) {
    ...
    if (!skipStaminaDeduction) {
        var currentStaminaCost = getCurrentBattleStaminaCost();
        if (stamina < currentStaminaCost && !isUnlimitedStaminaActive()) {
            showToast(`负荷不足！需要 ${currentStaminaCost} 点负荷`, 'error');
            return;
        }
        modifyStamina(-currentStaminaCost);
    }
    ...
    if (startBtn) startBtn.style.display = 'none';
    if (continueBtn) continueBtn.style.display = 'none';
    var autoBtn = document.getElementById('autoBattleBtn');
    ...
    // 缩小日志中的图片图标（替换2.8rem为1em）
    var smallIcon = (planet.icon || '🌍').replace(/width:\s*2\.8rem/g, 'width:1em').replace(/height:\s*2\.8rem/g, 'height:1em');
    addBattleLog(`🌍 抵达 ${smallIcon} ${planet.name || '未知区域'}！`, 'info');
...
function enemyDefeated() {
    ...
    BATTLE_DROPS.forEach(dropItem => {
        ...
        // 区域过滤
        if (dropItem.zoneFilter && dropItem.zoneFilter.length > 0) {
            var curZone = selectedZone ? selectedZone.id : null;
            if (!curZone || dropItem.zoneFilter.indexOf(curZone) === -1) return;
        }
        // 分离蓝图和普通掉落
    ...
    if (blueprintDrops.length > 0) {
        var bpChance = (typeof DROP_CONFIG !== 'undefined' && DROP_CONFIG.blueprint) ? (DROP_CONFIG.blueprint.bossChance || 0.10) : 0.10;
...
if (actualCashReward > 0) {
    ...
    if (battleStopRequested) {
        ...
        stopAutoBattle();
    } else if (autoBattleLoopActive) {
        // 自动循环模式：只重置 active 标志，不要恢复 UI，让下一场直接衔接
        autoBattleState.active = false;
        setTimeout(function() {
            if (!autoBattleLoopActive) return;
            var cost = getCurrentBattleStaminaCost() * 3;
            if (stamina >= cost || isUnlimitedStaminaActive()) {
                performAutoBattleOnce();
            } else {
                stopAutoBattleLoop();
                stopAutoBattle();
                showToast('⏹ 负荷耗尽，自动肃清已停止', 'warning');
                var aBtn = document.getElementById('autoBattleBtn');
                var sBtn = document.getElementById('stopBattleBtn');
...
function getEnemyXP(enemy) {
    ...
    return baseXP[cardType] || 100;
						...
						if (battleStopRequested) {
							...
							setTimeout(function() {
								...
								if (stamina >= cost || isUnlimitedStaminaActive()) {
									...
									stopAutoBattleLoop();
									showToast('⏹ 负荷耗尽，自动肃清已停止', 'warning');
									var aBtn2 = document.getElementById('autoBattleBtn');
									var sBtn2 = document.getElementById('stopBattleBtn');
									if (aBtn2) aBtn2.style.display = 'block';
					...
					function playerDefeated() {
						...
						// 肃清结束（失败）
						if (autoBattleLoopActive) {
							stopAutoBattleLoop();
							stopAutoBattle();
							showToast('⏹ 肃清失败，自动肃清已停止', 'warning');
							var aBtn3 = document.getElementById('autoBattleBtn');
							var sBtn3 = document.getElementById('stopBattleBtn');
							if (aBtn3) aBtn3.style.display = 'block';
							if (sBtn3) sBtn3.style.display = 'none';
						} else if (battleStopRequested) {
							stopAutoBattle();
							toggleAutoBattleMode(false);
							addBattleLog('🛑 连续肃清已停止', 'info');
						} else if (battleBtnPressState.autoMode) {
					...
					function stopAutoBattle() {
						...
						if (autoBtn5) {
							...
							var autoBtnText5 = document.getElementById('autoBattleBtnText');
							if (autoBtnText5) autoBtnText5.textContent = '🤖 自动(-' + autoCost5 + '⚡)';
						}
						if (continueBtn) {
							...
							var cost4 = getCurrentBattleStaminaCost();
							document.getElementById('continueBattleBtnText').textContent = '⚔️ 手动(-' + cost4 + '⚡)';
						}
						if (stopBtn) {
							stopBtn.style.display = 'none';
							stopBtn.textContent = '🛑 停止肃清';
					...
					function requestStopBattle() {
						...
						if (!autoBattleState.active) {
							...
							stopAutoBattle();
							addBattleLog('🛑 连续肃清已停止', 'info');
							var aBtn4 = document.getElementById('autoBattleBtn');
							var sBtn4 = document.getElementById('stopBattleBtn');
							if (aBtn4) aBtn4.style.display = 'block';
							if (sBtn4) sBtn4.style.display = 'none';
							return;
						}
					...
					function setBattlePortraitBackground(elementId, entity) {
						...
						const mirror = entity.mirror === true;
						el.classList.add('skill-combat-avatar');
						el.classList.remove('death-anim', 'casting', 'hit-shake', 'hit-flash', 'avatar-mirror');
						// 强制重置所有可能被动画forwards残留的样式
						...
						if (image) {
							...
							if (mirror) {
								// 镜像：用内层wrapper翻转图片，避免与CSS动画transform冲突
								el.style.backgroundImage = 'none';
								var wrap = document.createElement('div');
								wrap.className = 'avatar-mirror-wrap';
								wrap.style.cssText = 'position:absolute;inset:0;background-image:url(\'' + image + '\');background-size:contain;background-position:center;background-repeat:no-repeat;pointer-events:none;transform:scaleX(-1);';
								el.appendChild(wrap);
							} else {
					...
					function updateBattleUI() {
						if (!gameData) return;
						 						...
						 						// 更新负荷显示（当前值/自然上限100，无限负荷模式显示 ∞/xxx）
						 						    ...
						 						    // 蔚蓝商栈负荷
						 						    var corpusStaminaEl = document.getElementById('corpusStaminaValue');
						 						    if (corpusStaminaEl) {
						 						        corpusStaminaEl.textContent = (gameData?.stamina || stamina || 0) + '/100';
						 						    }
						...
						// 更新 Corpus 变体前缀轮换显示
						try {
						    var corPreStatus = getCorpusPrefixRotationStatus();
						    var corNameEl = document.getElementById('corpusPrefixName');
						    var corBuffEl = document.getElementById('corpusPrefixBuff');
						    if (corNameEl && corPreStatus.current) {
						        corNameEl.textContent = corPreStatus.current.theme;
						        corNameEl.style.color = corPreStatus.current.color || '#c8a84b';
						    }
						    if (corBuffEl && corPreStatus.current) {
						        corBuffEl.textContent = corPreStatus.current.buff || '';
						    }
						} catch (e) {}
...
function startVeinScan() {
    ...
    if (stamina < 10 && !isUnlimitedStaminaActive()) {
...
function startHeatAccumulation() {
    ...
    warframeMiningState.heatTimer = setInterval(() => {
        ...
        // ========== 新增：负荷耗尽检查 ==========
        if (stamina < 10 && !isUnlimitedStaminaActive() && warframeMiningState.active) {
...
function startPlantScan() {
   ...
   if (window._offlineLock) {
    ...
    if (stamina < 8 && !isUnlimitedStaminaActive()) { 
...
function stopAutoGathering() { stopInteractiveGathering(); }
    ...
    // 同步更新肃清页面负荷显示
					    ...
					    // 蔚蓝商栈负荷
					    var corpusStaminaUI = document.getElementById('corpusStaminaValue');
					    if (corpusStaminaUI) {
					        corpusStaminaUI.textContent = stamina + '/100';
					    }
					    // 寰宇-Corpus区域负荷
					    var corpusZoneStaminaUI = document.getElementById('corpusZoneStaminaValue');
					    if (corpusZoneStaminaUI) {
					        corpusZoneStaminaUI.textContent = stamina + '/100';
					    }
...
//  卡组星级奖励徽章更新
// ═══════════════════════════════════════════════════════════════
window.updateDeckRewardBadge = function() {
    if (!gameData || !playerCards) {
        console.log('[Badge] gameData 或 playerCards 为空');
        return;
    }
    ...
    for (var deckId in DECK_CARDS) {
        if (DECK_REWARD_DISABLED[deckId]) continue;
        if (canClaimDeckReward(deckId)) {
            claimableCount++;
        }
    }

    console.log('[Badge] 可领取奖励的卡组数:', claimableCount);
    ...
    if (badge) {
        if (claimableCount > 0) {
            ...
            console.log('[Badge] 显示徽章:', claimableCount);
        } else {
            badge.style.display = 'none';
            console.log('[Badge] 隐藏徽章');
        }
    } else {
        console.error('[Badge] 找不到 gatheringBadge 元素!');
    }
};
...
var MUSIC_PLAYLISTS = [
    ...
    },
    {
        ...
        icon: '🧱',
        cover: 'GAME/IMGs/music/2.jpg',
        tracks: [
            { title: 'Murum Vull', artist: '墙中低语 OST', src: 'GAME/IMGs/music/墙中低语/Murum Vull.mp3', duration: '3:55' },
    ...
    },
    {
        ...
        icon: '☮️',
        cover: 'GAME/IMGs/music/4.jpg',
        tracks: [
            { title: 'Ironclad', artist: '旧日和平 OST', src: 'GAME/IMGs/music/旧日和平/Ironclad.mp3', duration: '4:12' },
            { title: 'Lullaby of the Manifold (feat. Francesca Hauser)', artist: '旧日和平 OST', src: 'GAME/IMGs/music/旧日和平/Lullaby of the Manifold (feat. Francesca Hauser).mp3', duration: '3:55' },
...
function updateInfoUI() {
    ...
    // 新增：经验条
    const isMaxLevel = wfData.level >= 70;
            ...
            staminaBadge.style.color = 'var(--orokin-cyan)';
					...
					function addPlayerCard(cardData) {
					    ...
					    if (isNew) {
					        ...
					        if (typeof gainEchoFragment === 'function') {
					            var fragName = (ECHO_FRAGMENTS && ECHO_FRAGMENTS[fragRarity]) ? ECHO_FRAGMENTS[fragRarity].name : '碎片';
					            gainEchoFragment(fragRarity, 1);
					        }
					        // 重复获得时播放碎片小动画
					        if (typeof createFissureEffect === 'function') {
					            createFissureEffect();
					        }
					    }
					    ...
					    if (isNew) {
					        return { count: 1, isNew: true, converted: null };
					    } else {
					        return { count: playerCards[cardData.id].count, isNew: false, converted: null };
					    }
					}
...
function checkAndTriggerCodexReward() {
	...
	// ========== 关键修复：确保 playerCards 与当前用户一致 ==========
	    // 防止切换帐号后残留旧帐号数据
	    if (gameData.player_cards && playerCards !== gameData.player_cards) {
	        playerCards = gameData.player_cards;
	        window.playerCards = playerCards;
	    }
	    // ============================================================
...
function selectMiningPlanet(planetId) {
    ...
    if (zones && zones.length > 0) {
        ...
        if (log) {
            log.innerHTML = '';
            var smIcon1 = (planet.icon || '').replace(/width:\s*2\.8rem/g, 'width:1em').replace(/height:\s*2\.8rem/g, 'height:1em');
            addMiningLog('🌍 已抵达 ' + smIcon1 + ' ' + planet.name, 'info');
...
function selectGatheringPlanet(planetId) {
    ...
    if (zones && zones.length > 0) {
        ...
        if (log) {
            log.innerHTML = '';
            var smIcon2 = (planet.icon || '').replace(/width:\s*2\.8rem/g, 'width:1em').replace(/height:\s*2\.8rem/g, 'height:1em');
            addGatheringLog('🌍 已抵达 ' + smIcon2 + ' ' + planet.name, 'info');
...
function showMiningConfirm() {
    ...
    // 填充确认信息
    var miningConfirmIcon = document.getElementById('miningConfirmIcon');
    if (miningConfirmIcon) {
        if (selectedMiningZone.image) {
            miningConfirmIcon.innerHTML = '<img src="' + selectedMiningZone.image + '" style="width:2.8rem;height:2.8rem;object-fit:contain;filter:drop-shadow(0 0 6px rgba(200,168,75,0.5));">';
        } else if (selectedMiningZone.icon && selectedMiningZone.icon.indexOf('<img') !== -1) {
            miningConfirmIcon.innerHTML = selectedMiningZone.icon;
        } else {
            miningConfirmIcon.textContent = selectedMiningZone.icon || '';
        }
    }
...
function showGatheringConfirm() {
    ...
    // 填充确认信息
    var gatheringConfirmIcon = document.getElementById('gatheringConfirmIcon');
    if (gatheringConfirmIcon) {
        if (selectedGatheringZone.image) {
            gatheringConfirmIcon.innerHTML = '<img src="' + selectedGatheringZone.image + '" style="width:2.8rem;height:2.8rem;object-fit:contain;filter:drop-shadow(0 0 6px rgba(200,168,75,0.5));">';
        } else if (selectedGatheringZone.icon && selectedGatheringZone.icon.indexOf('<img') !== -1) {
            gatheringConfirmIcon.innerHTML = selectedGatheringZone.icon;
        } else {
            gatheringConfirmIcon.textContent = selectedGatheringZone.icon || '';
        }
    }
...
function backToGatheringPlanets() {
    ...
    renderGatheringPlanetSelect();
            ...
            <div style="background: var(--panel-bg); border: 1px solid ${isLocked ? '#333' : planet.color}; 
                        border-radius: 10px; padding: 15px; text-align: center; transition: all 0.3s; ${lockedStyle}"
                 ${onclick}>
                <div style="font-size: 2.5rem; margin-bottom: 8px; display:flex; align-items:center; justify-content:center;">${zone.icon}</div>
                <div style="font-family: 'Orbitron'; color: var(--tenno-gold); font-size: 1rem; margin-bottom: 4px;">${zone.name}</div>
                <div style="color: #888; font-size: 0.75rem; margin-bottom: 4px;">${zone.desc}</div>
    ...
    `;
					...
					function enterBattlePage() {
						...
						if (continueBtn) {
							...
							var cost2 = getCurrentBattleStaminaCost();
							if (btnText) btnText.textContent = '⚔️ 肃清(-' + cost2 + '⚡)';
						}
						if (stopBtn) stopBtn.style.display = 'none';
						...
						if (log) {
							log.innerHTML = '';
							var sIconP = (selectedPlanet.icon || '').replace(/width:\s*2\.8rem/g, 'width:1em').replace(/height:\s*2\.8rem/g, 'height:1em');
							if (selectedZone) {
							    var sIconZ = (selectedZone.icon || '').replace(/width:\s*2\.8rem/g, 'width:1em').replace(/height:\s*2\.8rem/g, 'height:1em');
							    addBattleLog(`🌍 已抵达 ${sIconP} ${selectedPlanet.name}`, 'info');
							    addBattleLog(`📍 ${sIconZ} ${selectedZone.name} - ${selectedZone.desc}`, 'info');
							} else {
							    addBattleLog(`🌍 已抵达 ${sIconP} ${selectedPlanet.name}，点击"继续肃清"开始肃清（-${getCurrentBattleStaminaCost()}⚡）`, 'info');
							}
						}
					}
        ...
        `;
					...
					function renderFactionZones(factionId, planet, zones) {
						...
						grid.innerHTML = zones.map(zone => {
							...
							const lockedStyle = isLocked ? 'opacity: 0.4; cursor: not-allowed; border-color: #333;' : 'cursor: pointer;';
							const onclick = isLocked ? '' : (hasSubZones ? `onclick="(typeof enterFactionSubZones==='function')?enterFactionSubZones('${factionId}','${zone.id}'):void(0)"` : `onclick="selectFactionZone('${factionId}','${zone.id}')"`);
        ...
        `;
					...
					function selectFactionZone(factionId, zoneId) {
						...
						var zoneIconEl = document.getElementById('selectedZoneIcon-' + factionId);
						if (zoneIconEl) zoneIconEl.innerHTML = zone.icon || '';
						...
						// 填充掉落预览（从 ZONE_DROP_TABLES 读取，统一格式）
						var dropsEl = document.getElementById('selectedZoneDrops-' + factionId);
						var dropsListEl = document.getElementById('selectedZoneDropsList-' + factionId);
						if (dropsEl && dropsListEl) {
						 var html = '';
						 try {
						     if (typeof ZONE_DROP_TABLES !== 'undefined' && ZONE_DROP_TABLES[factionId] && ZONE_DROP_TABLES[factionId][zoneId]) {
						         var zt = ZONE_DROP_TABLES[factionId][zoneId];
						         var allDrops = (zt.drops || []).concat(zt.rare || []);
						         for (var d = 0; d < allDrops.length; d++) {
						             var itemName = allDrops[d];
						             var isRare = (zt.rare || []).indexOf(itemName) !== -1;
						             var color = isRare ? '#ff66ff' : '#c8a84b';
						             html += '<span style="background:rgba(' + (isRare ? '255,102,255' : '200,168,75') + ',0.1);border:1px solid ' + color + ';color:' + color + ';padding:4px 10px;border-radius:12px;font-size:0.75rem;">' + itemName + '</span>';
						         }
						     }
						 } catch(e) {}
						 if (html) {
						     dropsListEl.innerHTML = html;
					...
					function confirmFactionBattle(factionId) {
						...
						// Corpus 走无尽回廊玩法（corpus_missions.js），只有战斗环节套用 Grineer 战斗系统
						if (factionId === 'corpus') {
							if (stamina < 20 && !isUnlimitedStaminaActive()) {
								showToast('负荷不足！需要 20 点', 'error');
								return;
							}
							modifyStamina(-20);
							if (typeof openCorpusMission === 'function') {
								openCorpusMission(null, selectedFactionPlanet.id, selectedFactionZone);
							} else {
								showToast('无尽回廊系统加载中...', 'warning');
							}
							return;
						}
					...
					// 确认派系区域肃清（Corpus 走无尽回廊，其他派系走普通肃清）
					function confirmFactionZoneBattle(factionId) {
						...
						// Corpus 走无尽回廊玩法（corpus_missions.js）
						if (factionId === 'corpus') {
							if (stamina < 20 && !isUnlimitedStaminaActive()) {
								showToast('负荷不足！需要 20 点', 'error');
								return;
						...
						// 非 Corpus 派系：不检查负荷，负荷由手动/自动按钮各自负责
						selectedPlanet = selectedFactionPlanet;
					...
					function enterBattlePage() {
						...
						// 自动肃清按钮只在寰宇-Grineer的游掠凶形/交锋异士/暴戾战将区域显示
						var autoBtn = document.getElementById('autoBattleBtn');
						if (autoBtn) {
							var showAuto = false;
							if (selectedFactionPlanet && selectedFactionPlanet.id === 'earth' && selectedFactionZone) {
								var autoZones = ['e_zone1', 'e_zone2', 'e_zone3'];
								if (autoZones.indexOf(selectedFactionZone.id) !== -1) {
									showAuto = true;
								}
							}
							if (showAuto) {
								autoBtn.style.display = 'block';
								var autoCost = getCurrentBattleStaminaCost() * 3;
								var autoText = document.getElementById('autoBattleBtnText');
								if (autoText) autoText.textContent = '🤖 自动(-' + autoCost + '⚡)';
							} else {
						...
						if (continueBtn) {
							...
							continueBtn.disabled = false;
							const btnText = document.getElementById('continueBattleBtnText');
							var cost3 = getCurrentBattleStaminaCost();
							if (btnText) btnText.textContent = '⚔️ 手动(-' + cost3 + '⚡)';
						}
						if (stopBtn) stopBtn.style.display = 'none';
						...
						if (log) {
							log.innerHTML = '';
							var sIconP2 = (selectedPlanet.icon || '').replace(/width:\s*2\.8rem/g, 'width:1em').replace(/height:\s*2\.8rem/g, 'height:1em');
							if (selectedZone) {
								var sIconZ2 = (selectedZone.icon || '').replace(/width:\s*2\.8rem/g, 'width:1em').replace(/height:\s*2\.8rem/g, 'height:1em');
								addBattleLog(`🌍 已抵达 ${sIconP2} ${selectedPlanet.name}`, 'info');
								addBattleLog(`📍 ${sIconZ2} ${selectedZone.name} - ${selectedZone.desc}`, 'info');
								
							} else {
								addBattleLog(`🌍 已抵达 ${sIconP2} ${selectedPlanet.name}，点击"继续肃清"开始肃清（-${getCurrentBattleStaminaCost()}⚡）`,
									'info');
							}
						}
					}
					...
					window.getCurrentWarframeData = getCurrentWarframeData;
...
function addXP(amount) {
    if (!gameData) { console.warn('[addXP] gameData 不存在'); return; }

    const type = gameData.warframe_type || 'excalibur';
    console.log('[addXP] 战甲类型:', type, '| 添加 XP:', amount);
    if (!gameData.warframe_levels) {
        gameData.warframe_levels = {};
    }
    if (!gameData.warframe_levels[type]) {
        ...
        console.log('[addXP] 创建 warframe_levels[' + type + ']:', JSON.stringify(gameData.warframe_levels[type]));
    }

    const wfData = gameData.warframe_levels[type];
    console.log('[addXP] 添加前:', 'level=' + wfData.level, 'xp=' + wfData.xp, 'max_xp=' + wfData.max_xp);
    
    // 等级上限70级
    const MAX_LEVEL = 70;
    
    // 数据修复：如果之前满级时 max_xp 被设为 0 或 999999，恢复为正常值
    if (wfData.level < MAX_LEVEL && (wfData.max_xp === 0 || wfData.max_xp >= 999999)) {
        ...
        wfData.xp = 0;
        console.log('[addXP] 数据修复: max_xp=' + wfData.max_xp, 'xp 重置为 0');
    }
    ...
    if (wfData.level >= MAX_LEVEL) {
        ...
        gameData.warframe_xp = wfData.xp;
        gameData.warframe_max_xp = wfData.max_xp;
        console.log('[addXP] 已达满级，不获得经验');
        return;
    }

    wfData.xp += amount;
    console.log('[addXP] 添加后 xp=' + wfData.xp);
    ...
    if (typeof updateInfoUI === 'function') updateInfoUI();
					...
					function getItemCategory(item) {
						...
						if (type === 'pack' || type === 'consumable') return 'consumable';

						// 成品战甲归类到加工品
						if (type === 'warframe') return 'product';
					...
					function addToWarehouse(name, icon, amount, type, image, packData) {
						...
						if (existing) {
							...
							if (packData) {
								existing.packData = packData;
												...
												if (packData) newItem.packData = packData;
					...
					function renderWarehouse() {
						...
						// 使用 window.warehouse 确保全局引用一致
						const currentWarehouse = window.warehouse || warehouse;
						if (window.warehouse !== warehouse) {
							warehouse = currentWarehouse;
						}

						// 按名称排序，并过滤掉无效/已耗尽物品
						const sortedWarehouse = [...currentWarehouse]
						...
						statsArea.innerHTML = `
						    <div style="display: flex; align-items: center; justify-content: space-between; width: 100%;">
						        <span style="color: #888; font-size: 0.85rem;">
							            ...
							            ${currentWarehouseCategory !== 'all' ? ` <span style="color: #555;">/ 总计 ${currentWarehouse.length} 项</span>` : ''}
							        </span>
						...
						if (filteredItems.length === 0) {
							...
							itemsGrid.innerHTML = filteredItems.map(item => {
								...
								const colors = { blueprint: '#4488ff', product: '#c8a84b', mineral: '#d4a574', material: '#4eff4e', pack: '#ff66ff', consumable: '#5adfff' };
								const color = colors[cat] || '#333';
								const label = WAREHOUSE_CATEGORIES.find(c => c.key === cat)?.label || '资源';
								var useBtn = '';
								var resolvedPackData = (typeof ensurePackData === 'function') ? ensurePackData(item) : item.packData;
								// 盲盒物品：获取对应一星类型样式
								var packStyleAttr = '';
								var packIconContent = '';
								if (resolvedPackData) {
									if (!item.packData) item.packData = resolvedPackData;
									var pd = resolvedPackData;
									var packCards = (typeof getDeckCardsForPack === 'function') ? getDeckCardsForPack(pd.deckId) : [];
									var packMaxRarity = 1;
									packCards.forEach(function(c) { if (c.rarity > packMaxRarity) packMaxRarity = c.rarity; });
									var fragType = (typeof FRAG_TO_CARD_TYPE !== 'undefined' && FRAG_TO_CARD_TYPE[packMaxRarity]) ? FRAG_TO_CARD_TYPE[packMaxRarity] : 'normal';
									var packStarType = (typeof CARD_STAR_STYLES !== 'undefined' && CARD_STAR_STYLES[fragType]) ? CARD_STAR_STYLES[fragType] : null;
									var packStar1 = packStarType && packStarType.stars ? packStarType.stars[0] : null;
									var packTypeColor = packStarType ? packStarType.color : '#ff66ff';
									if (packStar1) {
								...
								return `
									<div class="foundry-item" style="cursor: default; border-color: ${color}33; position: relative; overflow: hidden; ${packStyleAttr}">
		...
		function checkCanCraft(recipe) {
		    if (!recipe || !recipe.cost) return false;
		    var scaledCost = (typeof getScaledRecipeCost === 'function') ? getScaledRecipeCost(recipe) : recipe.cost;
		    for (const [material, amount] of Object.entries(scaledCost)) {
        ...
        return `
            <div class="foundry-item" style="border-color: ${borderColor}33; position: relative; overflow: hidden;">
                ...
                <div class="foundry-status" style="font-size: 0.8rem; line-height: 1.6;">
                    ${recipe.cost ? (() => {
                        var scaledCost = (typeof getScaledRecipeCost === 'function') ? getScaledRecipeCost(recipe) : recipe.cost;
                        var wfKey = recipe.warframeKey || '';
                        var mult = (wfKey && typeof WARFRAME_PART_COST_MULTIPLIERS !== 'undefined' && WARFRAME_PART_COST_MULTIPLIERS[wfKey]) ? WARFRAME_PART_COST_MULTIPLIERS[wfKey] : 1.0;
                        var showMult = mult > 1.0;
                        return Object.entries(scaledCost).map(([material, amount]) => {
                            const item = warehouse.find(w => w.name === material);
                            const have = item ? item.amount : 0;
                            const hasEnough = have >= amount;
                            var baseAmount = recipe.cost[material] || amount;
                            // 蓝图和已制造部件不显示倍率标注
                            var isPart = material.indexOf('蓝图') !== -1 || material.indexOf('头部神经光元') !== -1 || material.indexOf('机体') !== -1 || material.indexOf('系统') !== -1;
                            var isScaled = showMult && !isPart && amount !== baseAmount;
                            var amountText = isScaled ? `${amount}<span style="font-size:0.65rem;color:#888;"></span>` : amount;
                            return `<div style="color: ${hasEnough ? 'var(--infested-green)' : 'var(--grineer-red)'};">
                                ${material}: ${have}/${amountText}
                            </div>`;
                        }).join('');
                    })() : '<div style="color: #666;">无需材料</div>'}
                </div>
...
function craftItem(recipeId) {
    ...
    // 检查材料（应用战甲材料倍率）
    var scaledCost = (typeof getScaledRecipeCost === 'function') ? getScaledRecipeCost(recipe) : recipe.cost;
    if (!window.adminFreeCraftEnabled && scaledCost) {
        for (const [material, amount] of Object.entries(scaledCost)) {
            const item = warehouse.find(w => w.name === material);
...
function cancelFoundryCraft(craftKey) {
    ...
    sb.from('foundry_crafts')
        ...
        .then(function() {
            ...
            if (recipe) {
                var cancelCost = (typeof getScaledRecipeCost === 'function') ? getScaledRecipeCost(recipe) : recipe.cost;
                if (cancelCost) {
                    for (const [material, amount] of Object.entries(cancelCost)) {
                        addToWarehouse(material, '', amount, 'material');
                    }
                }
            }
    ...
    showToast(`${wf.name} 已存入矩阵`, 'success');
					...
					async function loadLeaderboard() {
						...
						try {
							...
							// 获取 admin 列表以排除（通过 clan_rank 或 username）
							var adminSet = {};
							try {
								var adminRes = await sb.from('game_users').select('user_id').eq('clan_rank', 'admin');
								(adminRes.data || []).forEach(function(u) { adminSet[u.user_id] = true; });
							} catch (e) {}
							...
							// 过滤掉 admin 用户（clan_rank=admin 或 username 包含 admin）
							var filteredData = data.filter(function(u) {
								if (adminSet[u.user_id]) return false;
								var name = (u.username || '').toLowerCase();
								if (name === 'admin' || name.indexOf('admin') >= 0) return false;
								return true;
							});

							// 计算每个用户的回响收集数量和战甲总等级
							const usersWithStats = filteredData.map(user => {
								let cardCount = 0;
							...
							tbody.innerHTML = usersWithStats.map((user, index) => {
								...
								else if (streakDays >= 40) { streakIcon = '💠'; streakColor = '#00ffff'; }
								else if (streakDays >= 30) { streakIcon = '🌟'; streakColor = '#ffd700'; }
								else if (streakDays >= 20) { streakIcon = '💎'; streakColor = '#00d4ff'; }
								else if (streakDays >= 10) { streakIcon = '🔥'; streakColor = 'var(--grineer-red)'; }
								const streakText = streakIcon + ' ' + streakDays + '天';

								// 总等级显示（每30级换色）
								var tl = user.totalLevel || 0;
								var tlColor;
								var tier = Math.floor(tl / 30);
								var tierColors = [
									'#888',       // 0-29: 灰
									'#65f0a3',    // 30-59: 绿
									'#5adfff',    // 60-89: 青
								...
								tlColor = tier >= tierColors.length ? 'var(--tenno-gold)' : tierColors[tier];
								...
								var rowBg = '';
								if (rank <= 3) {
								    var bgColors = { 1: 'rgba(255,215,0,0.08)', 2: 'rgba(192,192,192,0.06)', 3: 'rgba(205,127,50,0.06)' };
								    var bdColors = { 1: 'rgba(255,215,0,0.2)', 2: 'rgba(192,192,192,0.15)', 3: 'rgba(205,127,50,0.15)' };
								    rowBg = 'background:' + bgColors[rank] + '; border-left: 3px solid ' + bdColors[rank] + ';';
								}
    ...
    showConfirmModal({
        ...
        onConfirm: function() {
            ...
            var buffItem = {
                ...
                image: null,
                consumableData: {
                    staminaAmount: 'unlimited_1h',
                    description: '1小时无限负荷'
                }
            };
            addToWarehouse(buffItem.name, buffItem.icon, 1, 'consumable', null);
            var w = warehouse;
            for (var i = w.length - 1; i >= 0; i--) {
                if (w[i].name === buffItem.name && !w[i].consumableData) {
                    w[i].consumableData = buffItem.consumableData;
                    break;
                }
            }
            showToast('购买 1小时无限负荷卡！已放入矩阵', 'success');
...
function buyStaminaItem(name, price, type, staminaValue) {
    ...
    showConfirmModal({
        ...
        costs: [
            ...
            { label: '放入矩阵仓库', value: '📦 是', type: 'gain' },
        ],
        onConfirm: function() {
            ...
            // 统一放入矩阵仓库
            var itemIcon = type === 'buff' ? '🔥' : '⚡';
            var consumableData = type === 'buff'
                ? { staminaAmount: 'unlimited_1h', description: '1小时无限负荷' }
                : { staminaAmount: staminaValue, description: staminaValue === 'full' ? '负荷全满，上限+50' : '恢复' + staminaValue + '点负荷' };

            addToWarehouse(name, itemIcon, 1, 'consumable', null);
            var w = warehouse;
            for (var i = w.length - 1; i >= 0; i--) {
                if (w[i].name === name && !w[i].consumableData) {
                    w[i].consumableData = consumableData;
                    break;
                }
            }

            showToast(`购买 ${name}！已放入矩阵`, 'success');
            if (typeof renderWarehouse === 'function') renderWarehouse();
            saveGameData();
            updateUI();
        }
					    ...
					    if (type === 'consumable') {
					        var packItem = {
					            ...
					            consumableData: {
					                ...
					                description: staminaAmount === 'full' ? '负荷全满，上限+50' : '恢复' + staminaAmount + '点负荷'
					            }
					        };
					        addToWarehouse(packItem.name, packItem.icon, 1, 'consumable', null);
					        var w = warehouse;
					        for (var i = w.length - 1; i >= 0; i--) {
					            if (w[i].name === packItem.name) {
					                w[i].consumableData = packItem.consumableData;
					                break;
					            }
					        }
					        showToast(`购买 ${name}！已放入矩阵`, 'success');
...
const MAROO_BUY_ITEMS = [
    {
        name: 'EMBER·传家宝',
        icon: '',
        image: 'GAME/IMGs/items/EMBER.jpg',
    ...
    },
    {
        name: 'RHINO·传家宝',
        icon: '️',
        image: 'GAME/IMGs/items/RHINO.jpg',
    ...
    },
    {
        name: 'Valkyr·传家宝',
        icon: '',
        image: 'GAME/IMGs/items/Valkyr.jpg',
    ...
    },
    {
        name: 'VAUBAN·传家宝',
        icon: '',
        image: 'GAME/IMGs/items/VAUBAN.jpg',
                ...
                '<div class="heirloom-info">' +
                    ...
                    '<div class="heirloom-footer">' +
                        '<div class="heirloom-price">' +
                            '<img src="GAME/IMGs/申秉之魂.png" alt="" onerror="this.style.display=\'none\'">' +
                        '</div>' +
...
onConfirm: function() {
        ...
        <div style="display: grid; gap: 10px;">
            ...
            <div style="border-top:1px solid #333; padding-top:10px;">
                <div style="color:#aa44ff;font-size:0.8rem;margin-bottom:8px;">获得回响碎片（测试用）</div>
                <div style="display:grid; grid-template-columns:1fr 1fr 1fr; gap:6px;">
                    <button class="btn" style="background:#888;color:#000;font-size:0.75rem;" onclick="cheatAddFragment(1)">+10 遗落</button>
                    <button class="btn" style="background:#4488ff;color:#000;font-size:0.75rem;" onclick="cheatAddFragment(2)">+10 异变</button>
                    <button class="btn" style="background:#aa44ff;color:#000;font-size:0.75rem;" onclick="cheatAddFragment(3)">+10 古神</button>
                    <button class="btn" style="background:#ffaa00;color:#000;font-size:0.75rem;" onclick="cheatAddFragment(4)">+10 混沌</button>
                    <button class="btn" style="background:#ff2222;color:#000;font-size:0.75rem;" onclick="cheatAddFragment(5)">+10 终焉</button>
                    <button class="btn" style="background:#ff66ff;color:#000;font-size:0.75rem;" onclick="cheatAddFragment(0)">+10 全部</button>
                </div>
            </div>
            <div style="border-top:1px solid #333; padding-top:10px;">
                <div style="color:var(--tenno-gold);font-size:0.85rem;margin-bottom:8px;">🎬 敌人家族技能动画预览</div>
                <div style="display:flex;gap:8px;margin-bottom:8px;flex-wrap:wrap;">
                    <select id="adminPreviewFamily" onchange="adminUpdatePreviewSkills()" style="flex:1;min-width:120px;background:#1a1a2e;color:#ccc;border:1px solid #444;border-radius:6px;padding:6px 8px;font-size:0.8rem;">
                        <option value="">选择敌人家族...</option>
                        <option value="butcher">屠夫</option>
                        <option value="flameblade">烈焰刀客</option>
                        <option value="scorpion">天蝎</option>
                        <option value="shieldLancer">盾枪兵</option>
                        <option value="ballista">弩炮</option>
                        <option value="eviscerator">开膛者</option>
                        <option value="trooper">恶徒</option>
                        <option value="lancer">枪兵</option>
                        <option value="scorch">怒焚者</option>
                        <option value="seeker">追踪者</option>
                        <option value="hellion">骑兵</option>
                        <option value="bombard">轰击者</option>
                        <option value="mortarBombard">迫击炮轰击者</option>
                        <option value="napalm">火焰轰击者</option>
                        <option value="heavyGunner">重型机枪手</option>
                        <option value="commander">指挥官</option>
                        <option value="beastMaster">爪喀驯兽师</option>
                        <option value="beast">爪喀/鬣猫</option>
                        <option value="manic">狂躁者</option>
                        <option value="nox">毒化者</option>
                        <option value="vor">沃尔上尉</option>
                        <option value="corpusCrewman">船员</option>
                        <option value="corpusDetron">德特昂船员</option>
                        <option value="corpusElite">精英船员</option>
                        <option value="corpusVoid">虚能船员</option>
                        <option value="corpusSniper">狙击船员</option>
                        <option value="corpusTechnician">技师</option>
                        <option value="corpusMechanic">机械师</option>
                        <option value="corpusEvictor">驱逐员/扰敌员</option>
                        <option value="corpusBoss">引能船员/资料师</option>
                        <option value="corpusMoa">基础恐鸟</option>
                        <option value="corpusMoaReverse">逆进恐鸟</option>
                        <option value="corpusMoaLava">熔岩恐鸟</option>
                        <option value="corpusMoaRailgun">磁轨炮恐鸟</option>
                        <option value="corpusMoaShock">震荡恐鸟</option>
                        <option value="corpusMoaMini">微型恐鸟</option>
                        <option value="corpusMoaFusion">融合恐鸟</option>
                        <option value="corpusMoaDisc">圆盘恐鸟</option>
                        <option value="corpusMoaDera">德拉恐鸟</option>
                        <option value="corpusMoaTwin">双子炮恐鸟</option>
                        <option value="corpusMoaCryo">冷冻恐鸟</option>
                        <option value="corpusMoaElite">金流精英恐鸟</option>
                        <option value="corpusMoaDemo">爆破型恐鸟</option>
                    </select>
                </div>
                <div id="adminPreviewSkillButtons" style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:8px;"></div>
                <div id="adminPreviewArea" style="height:100px;background:rgba(0,0,0,0.3);border:1px dashed #444;border-radius:8px;display:flex;align-items:center;justify-content:center;color:#666;font-size:0.75rem;">选择家族后点击技能按钮预览动画</div>
            </div>
    ...
    `;
					...
					function cheatAddFragment(rarity) {
						if (typeof gainEchoFragment !== 'function') {
							showToast('回响系统未加载', 'error'); return;
						}
						if (rarity === 0) {
							for (var r = 1; r <= 5; r++) gainEchoFragment(r, 10);
							showToast('+10 全部碎片', 'success');
						} else {
							gainEchoFragment(rarity, 10);
							var fname = (ECHO_FRAGMENTS && ECHO_FRAGMENTS[rarity]) ? ECHO_FRAGMENTS[rarity].name : '碎片';
							showToast('+10 ' + fname, 'success');
						}
						saveGameData();
					}

					// ═══════ 测试面板：敌人家族技能动画预览 ═══════
					function adminUpdatePreviewSkills() {
						var sel = document.getElementById('adminPreviewFamily') || document.getElementById('testPreviewFamily');
						var box = document.getElementById('adminPreviewSkillButtons') || document.getElementById('testPreviewSkillButtons');
						if (!sel || !box) return;
						var family = sel.value;
						if (!family || typeof ENEMY_FAMILY_SKILLS === 'undefined' || !ENEMY_FAMILY_SKILLS[family]) { box.innerHTML = ''; return; }
						var skills = ENEMY_FAMILY_SKILLS[family];
						box.innerHTML = skills.map(function(sk, i) {
							return '<button class="btn" onclick="adminPreviewEnemySkillByName(\'' + family + '\',' + i + ')" style="border-color:#444;color:#ccc;font-size:0.75rem;padding:6px 10px;">' + (sk.icon || '⚔️') + ' ' + sk.name + '</button>';
						}).join('');
					}
					function adminPreviewEnemySkillByName(familyKey, skillIndex) {
						if (typeof ENEMY_FAMILY_SKILLS === 'undefined') return;
						var skills = ENEMY_FAMILY_SKILLS[familyKey]; if (!skills || !skills[skillIndex]) return;
						adminPreviewEnemySkill(skills[skillIndex]);
					}
					function adminPreviewEnemySkill(skill) {
						if (typeof spawnSkillVfx !== 'function') { showToast('动画系统尚未加载', 'error'); return; }
						// 先关闭测试面板模态层（避免backdrop-filter模糊VFX）
						if (typeof closeModal === 'function') {
							try { closeModal(); } catch(e) {}
						}
						var mo = document.getElementById('modalOverlay');
						if (mo) { mo.style.display = 'none'; mo.classList.remove('active'); }

						// 清理之前的预览元素
						['battleEnemyIcon','battlePlayerIcon'].forEach(function(id) { var el = document.getElementById(id); if (el && el._preview) el.remove(); });
						document.querySelectorAll('.skill-vfx').forEach(function(el) { if (el._previewVfx) el.remove(); });
						var oldOverlay = document.getElementById('adminPreviewOverlay');
						if (oldOverlay) oldOverlay.remove();

						// 创建半透明背景覆盖层（仿战斗环境）
						var overlay = document.createElement('div');
						overlay.id = 'adminPreviewOverlay';
						overlay.style.cssText = 'position:fixed;inset:0;z-index:999;background:rgba(0,0,0,0.6);display:flex;flex-direction:column;align-items:center;justify-content:center;pointer-events:none;';

						// 顶部标签
						var label = document.createElement('div');
						label.style.cssText = 'position:absolute;top:40px;left:50%;transform:translateX(-50%);font-family:Orbitron,sans-serif;color:var(--tenno-gold,#c8a84b);font-size:1rem;opacity:0.8;letter-spacing:2px;text-shadow:0 0 8px rgba(200,168,75,0.3);';
						label.textContent = '🎬 ' + skill.name + ' 技能预览';
						overlay.appendChild(label);

						document.body.appendChild(overlay);

						// 在屏幕中央放置fixed定位的源/目标元素（仿战斗布局）
						var battleY = window.innerHeight / 2;
						var centerX = window.innerWidth / 2;
						var srcX = centerX - 150;
						var tgtX = centerX + 150;

						var src = document.createElement('div'); src.id = 'battleEnemyIcon'; src._preview = true;
						src.style.cssText = 'position:fixed;left:' + srcX + 'px;top:' + battleY + 'px;width:64px;height:64px;margin-left:-32px;margin-top:-32px;background:linear-gradient(135deg,#2a2a3a,#4a4a5a);border-radius:50%;z-index:1000;display:flex;align-items:center;justify-content:center;font-size:32px;border:2px solid #666;box-shadow:0 0 15px rgba(0,0,0,0.5);';
						src.textContent = skill.icon || '🤖'; document.body.appendChild(src);
						var tgt = document.createElement('div'); tgt.id = 'battlePlayerIcon'; tgt._preview = true;
						tgt.style.cssText = 'position:fixed;left:' + tgtX + 'px;top:' + battleY + 'px;width:64px;height:64px;margin-left:-32px;margin-top:-32px;background:linear-gradient(135deg,#1a2a4a,#3a5a8a);border-radius:50%;z-index:1000;display:flex;align-items:center;justify-content:center;font-size:32px;border:2px solid #4488ff;box-shadow:0 0 15px rgba(0,0,0,0.5);';
						tgt.textContent = '🥷'; document.body.appendChild(tgt);

						// 短延迟确保DOM渲染完成
						setTimeout(function() {
							spawnSkillVfx('enemy', skill);
						}, 50);

						setTimeout(function() {
							if (src.parentNode) src.remove();
							if (tgt.parentNode) tgt.remove();
							document.querySelectorAll('.skill-vfx').forEach(function(el) { if (el._previewVfx) el.remove(); });
							var ol = document.getElementById('adminPreviewOverlay');
							if (ol) ol.remove();
						}, 3000);
					}

					// ========== 清理10天未登录账号（软删除）==========
					async function showInactiveCleanup() {
						if (!isAdmin()) { showToast('权限不足', 'error'); return; }
						var ac = document.getElementById('adminContent');
						if (!ac) return;
						ac.innerHTML = '<div style="text-align:center;color:#888;padding:20px;">查询中...</div>';
						try {
							var tenDaysAgo = new Date();
							tenDaysAgo.setDate(tenDaysAgo.getDate() - 10);
							// game_users 没有 last_login 字段，用 game_data.last_login
							var { data: inactive, error } = await sb
								.from('game_data')
								.select('user_id, last_login')
								.lt('last_login', tenDaysAgo.toISOString())
								.order('last_login', { ascending: true });
							if (error) throw error;
							if (!inactive || inactive.length === 0) {
								ac.innerHTML = '<div style="color:var(--infested-green);text-align:center;padding:20px;">没有10天以上未登录的账号</div>';
								return;
							}
							// 获取用户名
							var userIds = inactive.map(function(r) { return r.user_id; });
							var { data: users, error: uErr } = await sb
								.from('game_users')
								.select('id, username')
								.in('id', userIds);
							var userMap = {};
							if (users) users.forEach(function(u) { userMap[u.id] = u.username; });

							var html = '<div style="color:var(--grineer-red);margin-bottom:12px;">发现 ' + inactive.length + ' 个账号超过10天未登录：</div>' +
								'<div style="max-height:300px;overflow-y:auto;margin-bottom:12px;">';
							inactive.forEach(function(r) {
								var ll = r.last_login ? new Date(r.last_login).toLocaleDateString() : '未知';
								html += '<div style="display:flex;justify-content:space-between;padding:6px 10px;background:rgba(0,0,0,0.2);border-radius:6px;margin-bottom:4px;font-size:0.8rem;">' +
									'<span style="color:#fff;">' + (userMap[r.user_id] || r.user_id.substring(0,8)) + '</span>' +
									'<span style="color:#888;">最后登录: ' + ll + '</span></div>';
							});
							html += '</div>' +
								'<button class="btn" onclick="executeCleanup()" style="width:100%;background:linear-gradient(135deg,var(--grineer-red),#ff4444);color:#fff;">确认清理（软删除）</button>';
							ac.innerHTML = html;
						} catch (e) {
							ac.innerHTML = '<div style="color:var(--grineer-red);">查询失败: ' + e.message + '</div>';
						}
					}

					async function executeCleanup() {
						if (!confirm('确定要软删除这些账号吗？\n数据保留但账号将被标记为已删除。')) return;
						var ac = document.getElementById('adminContent');
						ac.innerHTML = '<div style="text-align:center;color:#888;padding:20px;">清理中...</div>';
						try {
							var tenDaysAgo = new Date();
							tenDaysAgo.setDate(tenDaysAgo.getDate() - 10);
							// 软删除 game_users（设 deleted_at）
							// 先获取要删除的 user_id 列表
							var { data: toDelete, error: findErr } = await sb
								.from('game_data')
								.select('user_id')
								.lt('last_login', tenDaysAgo.toISOString());
							if (findErr) throw findErr;
							if (!toDelete || toDelete.length === 0) {
								ac.innerHTML = '<div style="color:var(--infested-green);text-align:center;padding:20px;">没有需要清理的账号</div>';
								return;
							}
							var ids = toDelete.map(function(r) { return r.user_id; });
							var { error } = await sb
								.from('game_users')
								.update({ deleted_at: new Date().toISOString(), deletion_reason: '10天未登录自动清理' })
								.in('id', ids);
							if (error) throw error;
							ac.innerHTML = '<div style="color:var(--infested-green);text-align:center;padding:20px;">清理完成，已标记 ' + ids.length + ' 个账号</div>';
							showToast('清理完成', 'success');
						} catch (e) {
							ac.innerHTML = '<div style="color:var(--grineer-red);">清理失败: ' + e.message + '</div>';
						}
					}
					...
					// ===== 自动肃清按钮长按逻辑 =====
					var autoBattlePressState = { pressing: false, timer: null, longPressTriggered: false, interval: null };
					var autoBattleLoopActive = false;

					function handleAutoBattlePress(e) {
						if (e) { e.preventDefault(); e.stopPropagation(); }
						var btn = document.getElementById('autoBattleBtn');
						var progress = document.getElementById('autoBattlePressProgress');
						if (!btn) return;
						autoBattlePressState.pressing = true;
						autoBattlePressState.longPressTriggered = false;
						btn.classList.add('btn-long-pressing');
						if (progress) {
							progress.style.transition = 'width 0.1s linear';
							progress.style.width = '0%';
							setTimeout(function() { progress.style.width = '100%'; }, 30);
						}
						autoBattlePressState.timer = setTimeout(function() {
							autoBattlePressState.longPressTriggered = true;
							btn.classList.remove('btn-long-pressing');
							if (progress) { progress.style.width = '0%'; progress.style.transition = 'width 0.2s ease'; }
							// 长按：开始持续自动战斗
							startAutoBattleLoopWithSkills();
							// 隐藏自动按钮，只显示停止按钮
							var autoBtn2 = document.getElementById('autoBattleBtn');
							var stopBtn2 = document.getElementById('stopBattleBtn');
							if (autoBtn2) autoBtn2.style.display = 'none';
							if (stopBtn2) stopBtn2.style.display = 'block';
							showToast('🤖 持续自动肃清已启动（自动释放技能）', 'success');
						}, 800);
					}

					function handleAutoBattleRelease(e) {
						if (e) { e.preventDefault(); e.stopPropagation(); }
						var btn = document.getElementById('autoBattleBtn');
						var progress = document.getElementById('autoBattlePressProgress');
						autoBattlePressState.pressing = false;
						if (btn) btn.classList.remove('btn-long-pressing');
						if (autoBattlePressState.timer) { clearTimeout(autoBattlePressState.timer); autoBattlePressState.timer = null; }
						if (progress) { progress.style.width = '0%'; progress.style.transition = 'width 0.2s ease'; }
						// 短按：执行一次自动肃清
						if (!autoBattlePressState.longPressTriggered) {
							performAutoBattleOnce();
							// 单次自动肃清后显示停止按钮
							var stopBtn3 = document.getElementById('stopBattleBtn');
							if (stopBtn3) stopBtn3.style.display = 'block';
						}
					}

					function handleAutoBattleClick(e) {
						e.preventDefault();
						e.stopPropagation();
					}

					function performAutoBattleOnce() {
						// 防止战斗期间重复点击
						if (autoBattleState.active || (typeof SkillCombat !== 'undefined' && SkillCombat.isActive)) {
							showToast('肃清进行中，请等待结束', 'warning');
							return;
						}
						var cost = getCurrentBattleStaminaCost() * 3;
						if (stamina < cost && !isUnlimitedStaminaActive()) {
							showToast('负荷不足！自动肃清需要 ' + cost + ' 点', 'error');
							return;
						}
						// 扣除3倍负荷
						modifyStamina(-cost);
						// 隐藏自动按钮，显示停止按钮（进入手动肃清模式）
						var autoBtn = document.getElementById('autoBattleBtn');
						var stopBtn = document.getElementById('stopBattleBtn');
						if (autoBtn) autoBtn.style.display = 'none';
						if (stopBtn) stopBtn.style.display = 'block';
						// 开始自动战斗
						window.autoSkillMode = true;
						startAutoBattleWithPlanet(selectedPlanet, true);
					}

					function startAutoBattleLoopWithSkills() {
						autoBattleLoopActive = true;
						performAutoBattleOnce();
					}

					function stopAutoBattleLoop() {
						autoBattleLoopActive = false;
						if (autoBattlePressState.interval) {
							clearInterval(autoBattlePressState.interval);
							autoBattlePressState.interval = null;
						}
					}
					...
					function showSignInModal() {
						var overlay = document.getElementById('signInModalOverlay');
						if (!overlay) return;
					...
					async function renderSignInCalendar() {
						var serverDate = await getServerDate();
						var now = new Date(serverDate + 'T00:00:00+08:00');
						...
						var grid = document.getElementById('signInCalendarGrid');
						if (!grid) return;

						// 保留星期标题（前7个子元素）
						var headers = Array.from(grid.children).slice(0, 7);
						grid.innerHTML = '';
						...
						for (var i = 0; i < firstDay; i++) {
							...
							empty.style.cssText = 'padding: 8px 0; text-align: center; color: #444;';
							grid.appendChild(empty);
						}

						// 生成日期格子
						// 从 lastDate 和 streak 推导连续签到区间
						var signedStart = null;
						var signedEnd = null;
						var effectiveStreak = streak || 0;
						// 如果已签到但 streak 为 0（边缘情况），至少标记今天
						if (lastDate === serverDate && effectiveStreak < 1) effectiveStreak = 1;
						if (lastDate && effectiveStreak > 0) {
							signedEnd = new Date(lastDate + 'T00:00:00+08:00');
							signedStart = new Date(signedEnd.getTime());
							signedStart.setDate(signedStart.getDate() - (effectiveStreak - 1));
						}
						for (var d = 1; d <= daysInMonth; d++) {
							...
							if (signedStart && signedEnd) {
								...
								isSigned = dateObj >= signedStart && dateObj <= signedEnd;
							}
							var style = 'padding: 8px 0; text-align: center; border-radius: 8px; font-size: 0.85rem; font-family: "Orbitron"; cursor: default;';
							if (isToday && lastDate !== serverDate) {
								style += ' background: rgba(200,168,75,0.15); border: 1px solid var(--tenno-gold); color: var(--tenno-gold);';
							} else if (isSigned) {
								style += ' background: rgba(78,255,78,0.1); border: 1px solid rgba(78,255,78,0.3); color: var(--infested-green);';
								cell.innerHTML = '<span style="font-size:0.75rem;">✓</span>' + d;
							} else {
								style += ' background: rgba(255,255,255,0.03); border: 1px solid #333; color: #888;';
							}
							cell.style.cssText = style;
							if (!isSigned && !isToday) cell.textContent = d;
							if (isToday && lastDate === serverDate) cell.innerHTML = '<span style="font-size:0.75rem;">✓</span>' + d;
							grid.appendChild(cell);
						}

						// 更新今日奖励和按钮状态
						var claimBtn = document.getElementById('signInClaimBtn');
						if (lastDate === serverDate) {
							// 已签到：显示倒计时到明天北京时间 00:00
							if (claimBtn) { claimBtn.disabled = true; claimBtn.style.opacity = '0.6'; }
							updateSignInCountdown();
							// 启动倒计时刷新（如果尚未启动）
							if (!window._signInCountdownTimer) {
								window._signInCountdownTimer = setInterval(updateSignInCountdown, 60000);
							}
							document.getElementById('signInRewardContent').textContent = '今日奖励已领取';
						} else {
							if (claimBtn) { claimBtn.textContent = '✅ 立即签到'; claimBtn.disabled = false; claimBtn.style.opacity = '1'; }
							// 清除倒计时
							if (window._signInCountdownTimer) {
								clearInterval(window._signInCountdownTimer);
								window._signInCountdownTimer = null;
							}
							var recover = Math.max(0, Math.min(100, 1000 - (stamina || 0)));
							document.getElementById('signInRewardContent').textContent = '⚡ +' + recover + ' 负荷';
						}

						// 7天奖励区
						var weeklyArea = document.getElementById('signInWeeklyArea');
						if (weeklyArea) {
							if (streak > 0 && streak % 7 === 0) {
								var lastWeekly = (gameData && gameData.last_weekly_reward_date) || '';
								weeklyArea.style.display = 'block';
								var wBtn = document.getElementById('signInWeeklyClaimBtn');
								if (wBtn) {
									if (lastWeekly === serverDate) {
										wBtn.textContent = '已领取'; wBtn.disabled = true; wBtn.style.opacity = '0.6';
									} else {
										wBtn.textContent = '领取7天奖励'; wBtn.disabled = false; wBtn.style.opacity = '1';
									}
								}
							} else {
					...
					// 更新签到按钮倒计时
					function updateSignInCountdown() {
						var btn = document.getElementById('signInClaimBtn');
						if (!btn) return;
						// 如果按钮已启用（可以签到），不更新倒计时
						if (!btn.disabled) return;
						// 服务器 get_server_date 返回 UTC 日期，解析为 UTC+0 再加 8h 得北京时间
						// 即：UTC 00:00 = 北京时间 08:00 = 每日刷新点
						// 所以"下一个签到时间" = 下一个 UTC 00:00
						var now = new Date();
						var utcMs = now.getTime();
						var oneDayMs = 24 * 60 * 60 * 1000;
						// 当前 UTC 日期的 00:00
						var todayUtcMidnight = Math.floor(utcMs / oneDayMs) * oneDayMs;
						// 如果已过今天 UTC 00:00（即北京时间 08:00），目标是明天 UTC 00:00
						var nextRefresh = (utcMs >= todayUtcMidnight) ? todayUtcMidnight + oneDayMs : todayUtcMidnight;
						var remainMs = nextRefresh - utcMs;
						if (remainMs <= 0 || remainMs < 1000) {
							...
							// 异步查服务器确认日期已更新
							setTimeout(function() { renderSignInCalendar(); }, 1000);
							return;
						}
						var remainH = Math.floor(remainMs / (60 * 60 * 1000));
						var remainM = Math.floor((remainMs % (60 * 60 * 1000)) / (60 * 1000));
						btn.textContent = '✅ ' + String(remainH).padStart(2, '0') + ':' + String(remainM).padStart(2, '0') + ' 可再次签到';
					}
					async function handleSignInClaim() {
						await claimDailyLoginReward();
						renderSignInCalendar();
					}
					async function handleWeeklySignInClaim() {
						await claimWeeklyLoginReward();
						renderSignInCalendar();
					}
					...
					function toggleAutoBattleMode(enable) {
						...
						if (enable) {
							...
							var cost5 = getCurrentBattleStaminaCost();
							if (text) text.textContent = '⚔️ 肃清(-' + cost5 + '⚡)';
					...
					function startAutoBattleLoop() {
						...
						window.autoSkillMode = true;
						if (selectedPlanet && stamina >= getCurrentBattleStaminaCost() && !autoBattleState.active) {
							startAutoBattleWithPlanet(selectedPlanet);
						}
						...
						autoBattleLoopTimer = setInterval(() => {
							...
							if (!autoBattleState.active && selectedPlanet && stamina >= getCurrentBattleStaminaCost()) {
								if (battleStopRequested) {
									stopAutoBattleLoop();
									toggleAutoBattleMode(false);
									addBattleLog('🛑 自动巡航已停止', 'info');
								...
								startAutoBattleWithPlanet(selectedPlanet);
							} else if (stamina < getCurrentBattleStaminaCost() && !isUnlimitedStaminaActive() && !autoBattleState.active) {
					...
					function backToPlanetFromBattle() {
						...
						if (log) log.innerHTML =
							'<div style="color: #666; text-align: center; padding-top: 80px;">长按⚔️ 肃清(-5⚡) ️️消耗负荷进行连续肃清...</div>';
...
window.startAutoBattle = startAutoBattleWithPlanet;
					...
					window.renderSignInCalendar = renderSignInCalendar;
					window.updateSignInCountdown = updateSignInCountdown;
					...
					window.adminUpdatePreviewSkills = adminUpdatePreviewSkills;
					window.adminPreviewEnemySkillByName = adminPreviewEnemySkillByName;
					window.adminPreviewEnemySkill = adminPreviewEnemySkill;
					window.buyShopItem = buyShopItem;
					window.addToWarehouse = addToWarehouse;
					// useConsumableItem 和 openEchoPack 已在 echo_system.js 中暴露
					window.showInactiveCleanup = showInactiveCleanup;
					window.executeCleanup = executeCleanup;
					window.cheatAddFragment = cheatAddFragment;
					window.onRewardTypeChange = onRewardTypeChange;
					window.checkAdminAccess = checkAdminAccess;
					window.isAdmin = isAdmin;
					window.adminRestoreStamina = adminRestoreStamina;
					window.adminCancelUnlimitedStamina = adminCancelUnlimitedStamina;
...
window.adminPromoteCardStar = adminPromoteCardStar;
					...
					window.handleSignInClaim = handleSignInClaim;
					window.handleWeeklySignInClaim = handleWeeklySignInClaim;
					window.handleAutoBattleClick = handleAutoBattleClick;
					window.handleAutoBattlePress = handleAutoBattlePress;
					window.handleAutoBattleRelease = handleAutoBattleRelease;
					...
					window.renderFoundry = renderFoundry;
					...
					window.setVfxIntensity = setVfxIntensity;
					window.setUITheme = window.setUITheme || setUITheme;
					window.applyUITheme = window.applyUITheme || applyUITheme;
					window.renderThemeSelector = window.renderThemeSelector || renderThemeSelector;
					window.toggleDamageNumbers = toggleDamageNumbers;
